...
func (s *Service) SeedAlerts(orgID string, locations []string) {
	...
	demos := []Alert{
		// ── Predictive Maintenance (3) ──
		{
			...
			Description: "Nimbu El Gouna grill station #1 has been running 8°C above the 3-month average for the past 48 hours. Maintenance recommended within 48 hours to prevent equipment failure during peak service.",
			Status: "active", CreatedAt: now.Add(-2 * time.Hour),
			Metadata: map[string]any{"station": "Grill Station", "temp_delta_c": 8, "location": "Nimbu El Gouna"},
		},
		{
			...
			Description: "Nimbu New Cairo walk-in cooler compressor is running 23% longer per cycle compared to last week. This pattern preceded a failure at Nimbu El Gouna 3 months ago. Schedule inspection before Friday dinner service.",
			Status: "active", CreatedAt: now.Add(-4 * time.Hour),
			Metadata: map[string]any{"equipment": "Walk-in Cooler", "runtime_increase_pct": 23, "location": "Nimbu New Cairo"},
		},
		{
			...
			Description: "Nimbu Zayed dishwasher water pressure has dropped 15% over 2 weeks. Filter replacement typically resolves this. Last filter change was 45 days ago (recommended: 30 days).",
		...
		},
		{
			...
			Title:       "Beef Tenderloin usage variance +12% vs theoretical at Nimbu El Gouna",
		...
		// ── Financial Intelligence (3) ──
		{
			...
			Title:       "Nimbu New Cairo food cost trending 34.2% (target: 32%)",
			Description: "Food cost at Nimbu New Cairo has exceeded the 32% target for 8 consecutive days, driven by a 18% increase in sea bass and 15% in shrimp costs. Protein costs account for 68% of the overage. Recommend menu price review or supplier renegotiation.",
		...
		},
		{
			...
			Title:       "Nimbu North Coast labor cost at 29% — 3% above budget",
			Description: "Labor cost at Nimbu North Coast reached 29% of revenue this week, 3 percentage points above the 26% target. Overtime detected for 4 staff members (avg 12 extra hours each). Review scheduling or hire 2 additional part-time staff for weekend coverage.",
			Status: "active", CreatedAt: now.Add(-2 * time.Hour),
		...
		},
		{
			...
			Description: "Nimbu Zayed delivery channel gross margin fell to 18% (from 24% last month). Platform commission increased from 22% to 28% on the top delivery app. Consider renegotiating terms, adjusting delivery menu prices, or shifting volume to direct ordering.",
		...
		// ── Customer Intelligence (2) ──
		{
			...
			Description: "5 guests with average monthly spend of 2,400 EGP (total CLV: 12,000 EGP/month) have not visited any Nimbu location in 21+ days. Their visit frequency was previously bi-weekly. Recommend personalized re-engagement: complimentary Pisco Sour or Ceviche tasting invitation.",
			Status: "active", CreatedAt: now.Add(-1 * time.Hour),
		...
		},
		{
			...
			Title:       "Weekend dinner covers down 8% vs last month at Nimbu El Gouna",
			Description: "Friday and Saturday dinner covers at Nimbu El Gouna decreased 8% compared to the same period last month (avg 85 vs 92 covers). This deviates from the seasonal uptick pattern. Possible causes: competitor opening nearby, weather pattern change, or pricing sensitivity.",
		...
		{
			OrgID: orgID, LocationID: loc(1), Severity: SeverityWarning, Module: "operations",
			Title:       "Kitchen capacity at Nimbu New Cairo projected to exceed 90% during Friday dinner",
		...
		},
		{
			...
			Title:       "Average ticket time at Nimbu North Coast increased from 12 to 18 minutes",
			Description: "Average kitchen ticket time at Nimbu North Coast has increased 50% over the past week (12 min → 18 min). Analysis shows Ceviche Bar is the bottleneck — 73% of delayed tickets include ceviche items. Root cause: new cook on ceviche station with 2.5 ELU rating (team avg: 4.0).",
		...
		// ── Menu Intelligence (2) ──
		{
			...
			Description: "Churrasco Chimichurri has the highest margin (68%) in the menu but its 8-minute grill time limits throughput to 6 units/hour at Nimbu Zayed. During peak hours, it blocks 25% of grill capacity. Consider adding a dedicated churrasco grill or offering a pre-seared option.",
			Status: "active", CreatedAt: now.Add(-4 * time.Hour),
		...
		},
		{
			...
			Description: "Empanadas (3 pcs) sales velocity at Nimbu New Cairo increased 22% after the portion adjustment last month. Now selling 45 units/day (was 37). Classification upgraded from workhorse to crowd_pleaser. Margin remains healthy at 62%. Consider featuring as a starter special.",
		...
		// ── Vendor Intelligence (2) ──
		{
			...
			Description: "Metro Market's on-time-in-full rate at Nimbu North Coast has dropped from 85% to 72% over the past month. 3 of the last 5 deliveries arrived late or short. Recommend shifting 30% of produce orders to Seoudi Fresh (OTIF: 90%) while monitoring Metro Market performance.",
			Status: "active", CreatedAt: now.Add(-5 * time.Hour),
		...
		// ── Additional Financial/Ops (3) ──
		{
			...
			Title:       "Nimbu Zayed outperforming chain average by 12% on food cost",
			Description: "Nimbu Zayed is running a 30.8% food cost vs the chain average of 32.6%. Key driver: demand-based ceviche prep scheduling reduces fish waste by 22%. This best practice has been flagged for rollout to other locations.",
		...
		{
			OrgID: orgID, LocationID: loc(0), Severity: SeverityWarning, Module: "inventory",
			Title:       "Avocado waste rate at Nimbu El Gouna is 2.3x chain average",
			Description: "Nimbu El Gouna discarded 23 avocados in the past 14 days (chain average: 10). Primary cause: overripe at delivery (65%) and over-prepping for Causa Limena (35%). Recommend switching to 3x/week delivery for avocados and implementing FIFO labels.",
			Status: "active", CreatedAt: now.Add(-6 * time.Hour),
		...
		},
		{
			...
			Description: "Pisco Sour generates 72% margin at 185 EGP and sells 35+ units/day chain-wide. The upcoming 'Pisco Hour' campaign at Nimbu El Gouna saw 85 redemptions in 2 weeks. Recommend expanding to all 4 locations during the 5-7 PM window.",
